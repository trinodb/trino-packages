...
`downloadGithubTrinoPlugin` helper in `build.sh`. Trino no longer publishes
plugin zips to Maven Central as of Trino 477. The `downloadTrinoPlugin`,
`downloadGavPlugin`, and `downloadUrlPlugin` helpers remain available for
downloading plugins from a Maven repository or any other URL — useful when
plugins are mirrored to an internal Nexus or Artifactory.
...
```shell
docker run -p 8080:8080 --name trino-custom  trino-custom:483-arm64
```
...
The project is configured to build a custom image for Trino 483. Updates to
newer versions can be contributed to the repository or can be done locally. The
following steps are necessary:

* Update the parameter `TRINO_VERSION` set in `build.sh` to the desired Trino
  version.
* Confirm that the matching plugin zips are present on the corresponding
  [Trino GitHub release page](https://github.com/trinodb/trino/releases) and
  that the `trinodb/trino-core` Docker image has been published for the
  desired version and the target architectures.
